...
const getAnchorAndDir = (origin: RaysOrigin, w: number, h: number) => {
  const outside = 0.2;
  switch (origin) {
    case 'top-left':     return { anchor: [0, -outside * h] as [number,number], dir: [0.7, 0.7] as [number,number] };
    case 'top-right':    return { anchor: [w, -outside * h] as [number,number], dir: [-0.7, 0.7] as [number,number] };
    ...
    case 'bottom-left':  return { anchor: [0, (1 + outside) * h] as [number,number], dir: [0.7, -0.7] as [number,number] };
    case 'bottom-center':return { anchor: [0.5 * w, (1 + outside) * h] as [number,number], dir: [0, -1] as [number,number] };
    case 'bottom-right': return { anchor: [w, (1 + outside) * h] as [number,number], dir: [-0.7, -0.7] as [number,number] };
    ...
    const init = async () => {
      ...
      const frag = `
        ...
        float rayStrength(vec2 raySource,vec2 rayRefDirection,vec2 coord,float seedA,float seedB,float speed){
          ...
          float baseStrength=clamp((0.5+0.2*sin(distortedAngle*seedA+iTime*speed))+(0.3+0.2*cos(-distortedAngle*seedB+iTime*speed*0.8)),0.0,1.0);
          return baseStrength*lengthFalloff*fadeFalloff*spreadFactor*pulse;
        }
        void main(){
          vec2 fragCoord=gl_FragCoord.xy;
          vec2 finalRayDir=normalize(rayDir);
          if(mouseInfluence>0.0){vec2 mouseScreenPos=mousePos*iResolution.xy;vec2 mouseDirection=normalize(mouseScreenPos-rayPos);finalRayDir=normalize(mix(finalRayDir,mouseDirection,mouseInfluence));}
          float r1=rayStrength(rayPos,finalRayDir,fragCoord,45.2,31.4,0.8*raysSpeed);
          float r2=rayStrength(rayPos,finalRayDir,fragCoord,28.5,19.8,1.2*raysSpeed);
          float r3=rayStrength(rayPos,finalRayDir,fragCoord,12.1,56.2,0.5*raysSpeed);
          float combined=(r1*0.4+r2*0.4+r3*0.2);
          combined=pow(combined,0.7);combined*=1.5;
          vec3 finalColor=raysColor*combined;
          if(noiseAmount>0.0){float n=noise(fragCoord*0.01+iTime*0.05);finalColor*=(1.0-noiseAmount+noiseAmount*n);}
          if(saturation!=1.0){float gray=dot(finalColor,vec3(0.299,0.587,0.114));finalColor=mix(vec3(gray),finalColor,saturation);}
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const uniforms: any = {
        iTime:{value:0},iResolution:{value:[1,1] as [number,number]},
        rayPos:{value:[0,0] as [number,number]},rayDir:{value:[0,1] as [number,number]},
        ...
        saturation:{value:saturation},mousePos:{value:[0.5,0.5] as [number,number]},
